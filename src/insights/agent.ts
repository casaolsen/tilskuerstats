...
- Hvert værktøjssvar har et "ref"-felt (R1, R2, ...). Når du indsender et udkast, skal evidence pege på de refs, tallene kommer fra.
...
export type DraftInput = {
  ...
  body: string;
  match_ids: string[];
  evidence: { claim: string; ref: string }[];
};

export type Evidence = { claim: string; ref: string; tool: string; input: unknown; result: unknown };
...
export async function runAgent(deps: AgentDeps): Promise<AgentResult> {
  ...
  // ref ("R1", "R2", ...) -> hvad værktøjet blev kaldt med og svarede. Bruges til
  // at verificere udkast. Vi laver vores egne korte referencer og skriver dem
  // ind i selve svaret, fordi modellen ikke pålideligt kan se API'ets
  // tool_use-id'er - dem kan den derfor ikke citere.
  ...
  async function submitDraft(input: DraftInput): Promise<string> {
    ...
    const unknown = input.evidence.filter((e) => !toolLog.has(e.ref)).map((e) => e.ref);
    if (unknown.length) {
      const valid = [...toolLog].map(([ref, t]) => `${ref} (${t.tool})`).join(", ") || "ingen endnu";
      throw new Error(
        `evidence peger på ukendte refs: ${unknown.join(", ")}. ` +
          `Brug "ref"-feltet fra et værktøjssvar. Gyldige refs: ${valid}.`
      );
    }
    ...
    for (const e of input.evidence) {
      const badClaim = ungroundedNumbers(e.claim, [toolLog.get(e.ref)!.result]);
      if (badClaim.length) {
        throw new Error(`Påstanden "${e.claim}" har tal (${badClaim.join(", ")}), som ikke står i ${e.ref}.`);
      }
    }
    // Beviserne gemmes sammen med udkastet, så du kan se præcis hvad agenten så.
    const evidence = input.evidence.map((e) => ({ ...e, ...toolLog.get(e.ref)! }));
  ...
  async function executeTool(block: Anthropic.Beta.BetaToolUseBlock): Promise<{ content: string; isError: boolean }> {
    try {
      ...
      const ref = `R${toolLog.size + 1}`;
      toolLog.set(ref, { tool: block.name, input: block.input, result: output });
      return { content: JSON.stringify({ ref, ...(output as object) }), isError: false };
    } catch (e) {
